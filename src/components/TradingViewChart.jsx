  ...
  useEffect(() => {
    ...
    try {
      ...
      const volumeData = data.map(item => ({
        ...
        color: parseFloat(item.close) >= parseFloat(item.open) ? '#00ff8840' : '#ff444440',
      })).sort((a, b) => a.time - b.time);
        ...
        <div className="flex items-center space-x-4">
          ...
          {data.length > 0 && (
            <div className="flex items-center space-x-2 text-sm">
              ...
              <span className="text-white">{data[data.length - 1]?.open}</span>
              <span className="text-gray-400">H:</span>
              <span className="text-green-400">{data[data.length - 1]?.high}</span>
              <span className="text-gray-400">L:</span>
              <span className="text-red-400">{data[data.length - 1]?.low}</span>
              <span className="text-gray-400">C:</span>
              <span className={`font-medium ${
                parseFloat(data[data.length - 1]?.close) >= parseFloat(data[data.length - 1]?.open) 
                  ? 'text-green-400' 
                  : 'text-red-400'
              }`}>
