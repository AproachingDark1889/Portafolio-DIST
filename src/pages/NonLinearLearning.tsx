...
import { Brain, Zap, Filter, Search, ChevronDown, ChevronRight, CheckCircle, Lock, BookOpen, Settings, BarChart3, Network } from 'lucide-react';
...
const showToast = (
  ...
  toastElement.className = `fixed top-4 right-4 z-50 p-4 rounded-lg shadow-lg text-white max-w-sm transition-all duration-300 ${
    variant === 'destructive' 
...
const FragmentCard: React.FC<FragmentCardProps> = ({ fragment, onActivate, onSelect, isSelected }) => {
      ...
      {isExpanded && (
        <MaybeMotion initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-2 space-y-2">
          ...
          <button
            ...
            className={`w-full mt-2 px-4 py-2 rounded-lg font-medium transition-all ${fragment.activated ? 'bg-green-600/70 hover:bg-green-500/70 cursor-not-allowed' : 'bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700'} text-white`}
          >
...
const handleActivateFragment = useCallback((fragmentId: string): void => {
  ...
  const categories = useMemo(() => ['Todos', ...new Set(fragments.map(f => f.category))], [fragments]);
  ...
  const NetworkView = () => {
    ...
    const [currentNodePositions, setCurrentNodePositions] = useState<Array<Fragment & { x: number; y: number; categoryColor?: string }>>([]);
    ...
    const saveNodePosition = useCallback((id: string, x: number, y: number): void => {
      const positions = JSON.parse(localStorage.getItem('nn-positions') || '{}');
      positions[id] = { x, y };
      localStorage.setItem('nn-positissons', JSON.stringify(positions));
    }, []);
    
    const restorePositions = useCallback((nodes: Array<Fragment & { x: number; y: number; categoryColor?: string }>): Array<Fragment & { x: number; y: number; categoryColor?: string }> => {
      const positions = JSON.parse(localStorage.getItem('nn-positions') || '{}');
    ...
    const saveAllPositions = useCallback((): void => {
      if (layoutMode === 'force') {
        const positions: Record<string, { x: number; y: number }> = {};
        currentNodePositions.forEach(node => {
    ...
    const getFilteredFragments = useCallback((): Fragment[] => {
    ...
    const isNodeInView = useCallback((node: { x: number; y: number }, viewport: { width: number; height: number }): boolean => {
      const padding = 100;
      return (
        node.x * zoomLevel + dragOffset.x >= -padding &&
        node.x * zoomLevel + dragOffset.x <= viewport.width + padding &&
        node.y * zoomLevel + dragOffset.y >= -padding &&
        node.y * zoomLevel + dragOffset.y <= viewport.height + padding
      );
    ...
    // Mejora #7: Función para exportar imagen PNG/SVG (memoizada)
    const exportAsImage = useCallback(async (format = 'png') => {
      ...
      try {
        ...
        if (format === 'svg') {
          ...
          link.href = url;
          link.download = `neural-network-${new Date().getTime()}.svg`;
          link.click();
          ...
          img.onload = () => {
            ...
            ctx.fillStyle = '#1e293b'; // Fondo del tema
            ...
            canvas.toBlob((blob) => {
              ...
              const link = document.createElement('a');
              link.href = url;
              link.download = `neural-network-${new Date().getTime()}.png`;
              link.click();
              URL.revokeObjectURL(url);
            });
          };
    ...
    const handleKeyDown = useCallback((e) => {
      ...
      switch (e.key) {
        ...
        case '3':
          ...
          const layouts = ['circular', 'category', 'force'];
    ...
    const calculateNodePositions = useCallback(() => {
      ...
      if (layoutMode === 'force') {
        ...
        fragmentsToShow.forEach((fragment, index) => {
    ...
    const handleMouseDown = useCallback((e) => {
      if (e.target.tagName === 'circle') return; // No arrastrar si se hace clic en un nodo
      setIsDragging(true);
    ...
    const connections = useMemo(() => getConnections(), [getConnections]);
        ...
        >
          ...
          >
            ...
            {connections.map((conn, index) => {
              ...
              return (
                <g key={index}>
                  ...
                  <path
                    d={`M ${fromNode.x} ${fromNode.y} Q ${midX + controlOffset} ${midY - controlOffset} ${toNode.x} ${toNode.y}`}
            ...
            {currentNodePositions.map((node) => {
             ...
             // if (!isNodeInView(node, viewport)) return null;
        ...
        {showMiniMap && (
          <div className="absolute bottom-4 right-4 w-40 h-28 bg-black/90 border border-gray-600 rounded-lg overflow-hidden">
            ...
            >
              ...
              {currentNodePositions.map((node) => {
                const miniX = (node.x / 800) * 160;
                const miniY = (node.y / 600) * 96;
                return (
                  <circle
                    key={node.id}
                    cx={miniX}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <header className="fixed top-0 left-0 right-0 z-50 bg-slate-900/95 backdrop-blur-xl border-b border-slate-700/50 shadow-lg">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex bg-slate-800/50 rounded-lg p-1 border border-slate-700/50">
              <button
                onClick={() => handleViewChange('fragments')}
              ...
              >
                <Network className="w-4 h-4 mr-1 inline" />
                Red
              </button>
              <button
          ...
          {/* Barra de búsqueda y filtros - solo visible en vista de fragmentos */}
          {currentView === 'fragments' && (
            <div className="flex flex-col md:flex-row gap-4">
              <div className="relative flex-grow">
                <input
                  ...
                  className="w-full bg-slate-800/70 border border-slate-700 placeholder-gray-500 text-white pl-10 pr-4 py-2.5 rounded-lg focus:outline-none focus:border-purple-500"
                  value={searchTerm}
        ...
        {currentView === 'fragments' && (
          <div className="flex gap-6 h-[calc(100vh-220px)]">
            ...
            <div className="w-80 flex-shrink-0">
              {/* Lista de fragmentos con scroll */}
              <div className="h-full overflow-y-auto space-y-3 pr-2 scrollbar-thin scrollbar-thumb-slate-600/50 scrollbar-track-transparent">
                <AnimatePresence>
              ...
              {selectedFragment ? (
                ...
                >
                  ...
                  {selectedFragment.content ? (
                    <div className="space-y-6">
                      ...
                      {selectedFragment.content.theory && (
                        <div className="mb-6">
                          ...
                          <div className="prose prose-invert max-w-none p-4 bg-slate-800/50 rounded-lg border border-slate-700/50">
                            <div 
                              className="text-gray-300 text-sm leading-relaxed"
                              dangerouslySetInnerHTML={{ 
                                __html: selectedFragment.content.theory
                                  .replace(/```(\w+)?\n([\s\S]*?)```/g, (match, lang, code) => {
                                    const escapedCode = code
                                      .replace(/&/g, '&amp;')
                                      .replace(/</g, '&lt;')
                                      .replace(/>/g, '&gt;');
                                    return `<pre class="bg-slate-900 p-3 rounded mt-2 mb-2 overflow-x-auto"><code class="text-green-400 text-xs">${escapedCode}</code></pre>`;
                                  })
                                  .replace(/`([^`]+)`/g, (match, code) => {
                                    const escapedCode = code
                                      ...
                                      .replace(/</g, '&lt;')
                                      .replace(/>/g, '&gt;');
                                    return `<code class="bg-slate-700 px-1 py-0.5 rounded text-xs text-blue-300">${escapedCode}</code>`;
                                  })
                                  .replace(/\*\*(.*?)\*\*/g, '<strong class="text-white">$1</strong>')
                                  .replace(/\*(.*?)\*/g, '<em class="text-purple-300">$1</em>')
                                  .replace(/^### (.*$)/gim, '<h4 class="text-base font-semibold text-purple-300 mt-4 mb-2">$1</h4>')
                                  .replace(/^## (.*$)/gim, '<h3 class="text-lg font-semibold text-blue-300 mt-6 mb-3">$1</h3>')
                      ...
                      {selectedFragment.content.examples && selectedFragment.content.examples.length > 0 && (
                        <div className="mb-6">
                          ...
                          <div className="space-y-4">
                            {selectedFragment.content.examples.map((example, index) => (
                              <div key={index} className="bg-slate-800/50 rounded-lg border border-slate-700/50 overflow-hidden">
                                <div className="bg-slate-700/50 px-4 py-2 border-b border-slate-600/50">
                                  <h5 className="text-white font-medium text-sm">{example.title}</h5>
                                </div>
                                <pre className="p-4 overflow-x-auto">
                                  <code className="text-green-400 text-xs">{example.code}</code>
                                </pre>
                              </div>
                            ))}
                          </div>
                      ...
                      {selectedFragment.content.exercises && selectedFragment.content.exercises.length > 0 && (
                        <div className="mb-6">
                          ...
                          <div className="space-y-4">
                            {selectedFragment.content.exercises.map((exercise, index) => (
                              <div key={index} className="bg-slate-800/50 rounded-lg border border-slate-700/50 p-4">
                                <h5 className="text-white font-medium mb-2">{exercise.title}</h5>
                                <p className="text-gray-300 text-sm mb-3">{exercise.description}</p>
                                <details className="group">
                                  <summary className="cursor-pointer text-purple-400 text-sm font-medium mb-2">
                                    📋 Ver pasos detallados
                                  </summary>
                                  <div className="mt-2 pl-4 border-l-2 border-purple-500/30">
                                    {exercise.steps && exercise.steps.map((step, stepIndex) => (
                                      <p key={stepIndex} className="text-gray-400 text-xs mb-1">{step}</p>
                                    ))}
                                    {!exercise.steps && (
                                      <p className="text-gray-400 text-xs mb-1">Ejercicio en desarrollo - pasos disponibles próximamente.</p>
                                    )}
                                    {exercise.hint && (
                                      <div className="mt-3 p-3 bg-yellow-900/20 border border-yellow-500/30 rounded">
                                        <p className="text-yellow-300 text-xs">💡 <strong>Pista:</strong> {exercise.hint}</p>
                                      </div>
                                    )}
                                  </div>
                                </details>
                      ...
                      {selectedFragment.content.resources && selectedFragment.content.resources.length > 0 && (
                        <div className="mb-6">
                          ...
                          <div className="space-y-2">
                            {selectedFragment.content.resources.map((resource, index) => (
                              <a 
                                key={index} 
                                href={resource.url} 
                                target="_blank" 
                                rel="noopener noreferrer"
                                className="block p-3 bg-slate-800/50 rounded-lg border border-slate-700/50 hover:border-blue-500/50 transition-colors"
                              >
                                <h5 className="text-blue-400 font-medium text-sm">{resource.title}</h5>
                      ...
                      {selectedFragment.content.tips && selectedFragment.content.tips.length > 0 && (
                        <div className="mb-6">
                          ...
                          <div className="space-y-2">
                            {selectedFragment.content.tips.map((tip, index) => (
                              <div key={index} className="p-3 bg-gradient-to-r from-blue-900/20 to-purple-900/20 rounded-lg border border-blue-500/20">
                    ...
                    /* Fallback para fragmentos sin contenido detallado */
                    <div className="mb-6">
                      ...
                      <div className="p-4 bg-slate-800/50 rounded-lg text-gray-300 text-sm min-h-[150px] border border-slate-700/50">
                        <p>📝 <strong>Contenido en desarrollo...</strong></p>
                        <p className="mt-2">Este fragmento aún no tiene contenido detallado implementado. Próximamente incluirá:</p>
                        <ul className="list-disc list-inside ml-4 mt-2 space-y-1">
                          <li>Explicaciones teóricas profundas</li>
                          <li>Ejemplos de código prácticos</li>
                          <li>Ejercicios paso a paso</li>
                          <li>Recursos y referencias externas</li>
                          <li>Tips y mejores prácticas</li>
                        </ul>
                        <div className="mt-4 p-3 bg-blue-900/20 border border-blue-500/30 rounded">
                          <p className="text-blue-300 text-xs">💡 <strong>Mientras tanto:</strong> Usa el resumen arriba como guía inicial y explora los recursos online relacionados con "{selectedFragment.title}".</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {!selectedFragment.activated && (
                    <button
                      ...
                      className="w-full mt-6 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 px-4 py-2 rounded-lg text-white font-medium"
                    >
